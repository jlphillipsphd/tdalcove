...
CoarseCodeLayer::CoarseCodeLayer() : Layer() {
	cc_func = NULL;
	cc_work_vec = NULL;
	cc_work_vec2 = NULL;
}

CoarseCodeLayer::~CoarseCodeLayer() {
...
CoarseCodeLayer::CoarseCodeLayer(ActivationFunction* coarse_code_function,
			            int* number_of_units_per_dimension,
				        double* minimums_per_dimension,
				        double* maximums_per_dimension,
				        ActivationFunction* af) : 
						Layer(computeUnits(coarse_code_function,
							number_of_units_per_dimension), af) {
...
int CoarseCodeLayer::computeUnits(ActivationFunction* af, int* vals) {
	if (vals == NULL)
		return 0;
...
double CoarseCodeLayer::computeStep(int n, double min, double max) {
	double scale = fabs(max - min);
	return (scale / ((double) (n-1)));
}

bool CoarseCodeLayer::setCoarseActs(double* position) {
	if (cc_func == NULL)
		return false;
...
bool CoarseCodeLayer::setCoarseTargets(double* position) {
	if (cc_func == NULL)
		return false;
...
bool CoarseCodeLayer::setCoarseNets(double* position) {
...
void CoarseCodeLayer::codeLocation(int num_dims, int* dims, int* pos, int index) {
